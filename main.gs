...
const MESSAGE_FORM_FULL = ""; // only applies when RESPONSE_LIMIT is set. Leave blank ("") to disable.
...
const OPEN_DATETIME = "";
const CLOSE_DATETIME = "";

const MESSAGE_BEFORE_OPEN = ""; // only applies when OPEN_DATETIME is set. Leave blank ("") to disable.
...
function init() {
    ...
    try {
        if (OPEN_DATETIME != "") {
            ...
            ScriptApp.newTrigger("openForm_")
                ...
                .create();
            Logger.log(
                `Trigger to open form at ${OPEN_DATETIME} added by init()`
            );
            if (MESSAGE_BEFORE_OPEN != "")
                setClosedFormMessage_(MESSAGE_BEFORE_OPEN);
        }
        ...
        if (CLOSE_DATETIME != "") {
            ...
            ScriptApp.newTrigger("closeForm_")
                ...
                .create();
            Logger.log(
                `Trigger to close form at ${CLOSE_DATETIME} added by init()`
            );
        }
        ...
        if (EMAIL_SEND_AT != "") {
            ...
            ScriptApp.newTrigger("sendReminderEmail_")
                ...
                .create();
            Logger.log(
                `Trigger to send reminder email at ${EMAIL_SEND_AT} added by init()`
            );
            Logger.log(
                `> NOTE: Reminder Email will be sent from ${USER_EMAIL_ADDR}`
            );
        }
        ...
        if (RESPONSE_LIMIT != -1) {
            if (RESPONSE_LIMIT <= 0)
                throw new Error(
                    "RESPONSE_LIMIT cannot be less than or equal to 0"
                );
            ScriptApp.newTrigger("limitResponses_")
                ...
                .create();
            Logger.log(
                `Trigger to limit form to ${RESPONSE_LIMIT} responses added by init()`
            );
        }
    } catch (error) {
        removeAllTriggers();
    ...
    Logger.log(
        `> NOTE: To remove all triggers, run removeAllTriggers() function`
    );
...
function limitResponses_() {
    ...
    if (currentCount >= RESPONSE_LIMIT) {
        ...
        Logger.log(
            `Limit of ${RESPONSE_LIMIT} reached. The form has been closed.`
        );
    }
}
